...
首先，对令牌生成器进行更改，以使其sizeof将关键字TK_SIZEOF识别为类型的令牌。
...
前两名的Foo变量定义和底部的两个是函数定义，但它们可以被区分，直到到达函数名称或成为变量的名称和读取下一个标记标识。不。所以，你需要首先调用“读取类型名称上半年”功能，然后读取标识符，它应该来后，再向前看一个令牌。如果预读标记为“（”，则您正在读取函数定义，否则您正在读取变量定义。
...
首先调整令牌生成器，找到双引号后，阅读下一个双引号以创建字符串令牌。您无需在此步骤中实现反斜杠转义等。循序渐进很重要，因此，如果您认为易于实现，请不要这样做。
...
在语法上，注释与单个空格字符相同。因此，很自然地以与空白字符相同的方式在令牌生成器中跳过注释。跳过注释的代码如下所示。
...
链接器默认会尝试动态链接，但是编译器必须输出可以执行此操作的汇编代码。9cc目前不会输出这样的代码，因此，如果-static您忘记添加它，则会收到上述错误。通过阅读本章，您应该能够理解上述错误的含义以及解决这些错误所需采取的措施。
...
9cc假设源文件位于一个名为您的主目录的子目录中。make test要从该目录的容器中运行，请运行以下命令：
